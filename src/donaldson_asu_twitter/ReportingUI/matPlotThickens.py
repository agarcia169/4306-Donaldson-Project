...
#grabs all tweets with the powertrain value that was passed to it, maybe then finds the count for it
specific_powertrain_mention_count(arg) =  dbConnection.query_specific_powertrain_mention_count(arg)
thisDBClient = dbConnection.get_db_connection()
with thisDBClient.cursor() as dbCursor:
    dbCursor.execute(specific_powertrain_mention_count(arg))
    uniquePowertrains = dbCursor.fetchall()
...
#FIXME
query_specific_powertrain_mention_count = """ SELECT * FROM tweets WHERE powertrain_set LIKE "%d" """